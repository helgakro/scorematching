...
nresmesh3d <- rep(0,21)
reslistdense <- vector("list", 21)
nresmeshdense <- rep(0,21)
nlist <- c(100,200,500,1000,1500,2000)

nlist <- exp(seq(2,7,0.7))#exp(seq(2,8,0.7))
times_df <- data.frame()
...
#  nresmesh3d[i]<-n#mesh_sim1d$n
  ...
  if(mesh_sim$n<exp(5)){
    ...
    m<-t(inla.qsample(n=1, Q = Q, mu=mu)) #10
    Qdense <- as.matrix(Q)
    mbtestdense<-microbenchmark::microbenchmark(
      loo_score_vectorised(m,mu,Qdense,score="sroot"),
      log_dmvn(m,mu,Qdense),
      times=10)
    times_df <- rbind(times_df,data.frame(summary(mbtestdense,unit="ms"),n=mesh_sim$n,dim="2d dense"))
  }
...
nlist <- exp(seq(5,9,0.7))#exp(seq(2,8,0.7))
times_df <- data.frame()
...
levels(times_df$expr)<-c("Sroot","LL")
times_df$expr<-factor(times_df$expr,c("LL","Sroot"))

save(times_df, file=paste("m2_different_mesh_",format(Sys.time(), "%Y%m%d_%H%M%S"),"_reslist_diffdim.Rda"))
ggplot(data=times_df,aes(x=log(n),y=log(mean),color=expr,shape=expr))+geom_point()+
  geom_smooth(data=subset(times_df,log(n)>4),method = "lm", se = FALSE, aes(fill=expr,linetype=expr),alpha=0.3)+scale_fill_manual(values=c("#95D840FF","#2D708EFF","#95D840FF","#2D708EFF","#95D840FF","#2D708EFF","#3CBB75FF","#39568CFF"))+scale_color_manual(values=c("#95D840FF","#2D708EFF","#95D840FF","#2D708EFF","#95D840FF","#2D708EFF","#3CBB75FF","#39568CFF"))+scale_shape_manual(values=c(19,19,17,17,15,15))+scale_linetype_manual(values=c("solid","dotted","solid","dotted","solid","dotted"))+
  ylab("log(mean runtime)")
...
ggplot(data=times_df,aes(x=n,y=mean,color=expr,shape=expr))+geom_point()+
  geom_smooth(data=subset(times_df,log(n)>5),method = "lm", se = FALSE, aes(fill=expr,linetype=expr),alpha=0.3,size=0.5)+scale_fill_manual(values=c("#1B9E77", "#D95F02","#1B9E77", "#D95F02","#1B9E77", "#D95F02"))+scale_color_manual(values=c("#1B9E77", "#D95F02","#1B9E77", "#D95F02","#1B9E77", "#D95F02"))+scale_shape_manual(values=c(19,17,19,15,15))+scale_linetype_manual(values=c("solid","solid","dashed","dashed"))+
  scale_x_continuous(trans='log10') +
  scale_y_continuous(trans='log10') +ylab("time (ms)")
ggsave(paste("m2_different_mesh_",format(Sys.time(), "%Y%m%d_%H%M%S"),"_reslist_diffdim.pdf"),dpi = 1200,width = 12,height = 8,units = 'cm')
